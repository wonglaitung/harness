...
## 2026-07-17: 技术规范文档应先于问题记录

### 问题

`development_guide.md` 长期作为问题集记录，开发者遇到问题后才补充内容，缺乏预防性指导。
...
**关键变化**：
1. **先定义规范**：明确"必须做什么"，而非被动记录"遇到什么问题"
2. **强制性语言**：使用"禁止"、"必须"，而非"建议"
3. **完整检查清单**：每种场景都有检查项，便于合规审计
4. **问题参考降级为附录**：历史问题保留但不占据主导位置

### 教训

1. **规范先于问题**：文档应指导开发，而非只是事后记录
2. **强制性 > 建议性**：技术规范要使用明确的语言，避免"建议"、"推荐"等模糊表述
3. **检查清单是核心**：完整的检查清单支持合规审计
4. **版本号表示演进**：规范文档要有版本号，追踪演进历史

### 关键文件

| 文件 | 改动 |
|------|------|
| `packages/client/docs/development_guide.md` | 从问题集格式重构为技术规范格式（版本 2.0 → 3.0） |
