...
class AgentHarness:
    ...
    async def stream(
        ...
        """
        Stream the agent's response as typed StreamEvent objects.

        Events follow a structured envelope (source/category/seq) for
        composable consumers. Text chunks arrive as type="text" with
        source="agent" and category="text".

        Args:
            prompt: User input
            session_id: Optional session ID for conversation continuity
            on_chunk: Optional callback for each text chunk (raw string)
            on_progress: Optional callback for progress events
            verbose: If True, print progress to console

        Yields:
            StreamEvent with source="agent", category="text" for text chunks
