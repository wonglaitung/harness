...
async def demo_write_layers():
    ...
    item_additive = BlackboardItem(
        type="proposal",
        content={"suggestion": "增加单元测试覆盖"},
        source_agent="junior-dev",
        kind=WriteKind.ADDITIVE,  # ← additive 层
    )
    await store.put(item_additive)
    print(f"[additive] junior-dev 提议: {item_additive.content}")

    # authoritative 写入：需 provenance
    item_auth = BlackboardItem(
        type="decision",
        content={"action": "approve"},
        source_agent="tech-lead",
        kind=WriteKind.AUTHORITATIVE,  # ← authoritative 层
        provenance="meeting:2024-01-15:decision-1",  # A4: 必填
    )
    await store.put(item_auth)
...
async def demo_cas_concurrency():
    ...
    print(f"[Agent A] 写入初始版本: value=100, version={item.version}")
    ...
    ok, updated = await store.write_if_version(
        "counter", {"value": 200}, base_version=1  # 当前版本
    )
    print(f"[Agent C] CAS 写入 (base_version=1): {'成功' if ok else '失败'}")
    if ok:
...
async def demo_conflict_detection():
    ...
    await store.put(item_a)
    print(f"[Agent A] 决策: {item_a.content}")
    ...
    await store.put(item_b)
    print(f"[Agent B] 决策: {item_b.content}")
...
async def demo_real_scenario():
    ...
    # 1. Analyzer 写入分析结果
    await store.put(BlackboardItem(
        type="analysis",
        content={
            "modules": ["auth", "api", "db"],
            "complexity": "medium",
    ...
    await store.put(BlackboardItem(
        ...
        content={
            "errors": 0,
            "warnings": 3,
            "issues": ["unused-import", "line-too-long", "missing-docstring"],
        },
        source_agent="linter",
        writer_id="harness",
    ))
