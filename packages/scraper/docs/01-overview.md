...
构建一个**通用信息提取代理 + 技能驱动**系统：

- **通用代理**：极简的 base prompt，只定义角色和通用原则
- **技能驱动**：工具选择、工作流程、判断标准全部由 skill 文件定义
- **遵循 Bitter Lesson**：不硬编码控制流，让模型根据 skill 自主决策

### 核心设计原则

| 原则 | 说明 |
|------|------|
| **通用代理** | base prompt 不包含具体工具或工作流程 |
| **技能驱动** | skill 文件定义一切：工具清单、判断标准、输出模板 |
| **Bitter Lesson** | 不硬编码"先做 A 再做 B"，让模型自己规划 |
| **最小默认** | 无 skill 时只有 `fetch_url`，避免工具污染 |
