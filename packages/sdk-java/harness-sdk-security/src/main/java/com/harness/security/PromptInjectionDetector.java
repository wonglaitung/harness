...
public class PromptInjectionDetector {
    ...
    public static final List<String> DEFAULT_INJECTION_PATTERNS = List.of(
        ...
        "忘(记|掉|記|卻|却)(以上|之前|前述|上面|先前)?(的)?(指令|指示|要求|提示|设定|設定|限制|规则|規則|约束|約束)",
        "不要(理会|理睬|管|搭理)(以上|之前|前面|先前)?(的)?(指令|指示|要求|提示|限制|规则)",
        "把(上面|之前|以上)(的)?(指令|指示|要求|提示)(全部)?(抛|丢|扔)到(一|脑)边",
        // ---- Chinese: 系统提示泄露 ----
        "(告诉|展示|显示|透露|念出|重复|说出|說出|拷贝|複製)(我)?(你的)?(系统|初始|system)?(提示|指令|prompt)",
        "你的(系统|初始|system)?(提示|指令|prompt)(是(什么|啥)|内容|是什么)",
        "(输出|打印|顯示|输出)(你(的)?(系统|初始|system)?(提示|指令|prompt))",
        "(泄露|泄漏|透露)(你(的)?)(系统|初始|system)(提示|指令|prompt)",
        // ---- Chinese: 越狱 / 角色扮演 ----
        "假装(你|我)是",
        "假设(你|我)是",
        "扮演(一个|一名|一种|一个|一種|a|an|成)",
        "(现在|現在)你(就)?(是|变成|變成).{0,10}(限制|约束|約束|规则|規則|安全)",
        "越(狱|獄)",
        // ---- Chinese: 危险指令 ----
        "删除(所有|全部|一切|全部)?(的)?(文件|数据|資料|记录|記錄|资料)",
        "格式化(磁盘|硬盘|磁碟|系统)",
        "(运行|执行|執行)(以下|下列|这个|這個|恶意|惡意)?(的)?(命令|指令|脚本|腳本)",
        "(解密|解碼|解码)(后|後)(运行|执行|執行)"
    );
