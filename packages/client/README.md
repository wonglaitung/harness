...
| 📂 **统一配置** | 所有配置和数据集中存储在 `~/.harness/` 目录 |
| ⚙️ **设置持久化** | 配置自动保存，重启后自动加载 |
| 🎨 **Hermes 暗色主题** | 现代化暗色 UI 风格 |

### 数据存储位置

客户端**只读取** `~/.harness/` 目录，跨平台统一路径：

| 文件/目录 | 说明 |
|-----------|------|
| `~/.harness/settings.json` | 客户端设置（API Key、模型等） |
| `~/.harness/mcp.json` | MCP 服务器配置 |
| `~/.harness/MEMORY.md` | 全局记忆 |
| `~/.harness/skills/` | 用户技能目录 |

**跨平台路径**：
- Windows: `C:\Users\<用户名>\.harness\`
- Linux/macOS: `/home/<用户名>/.harness/`
...
| 目录 | 说明 |
|------|------|
| `~/.harness/skills/` | 用户级（所有项目共享，UI 创建默认保存位置） |
